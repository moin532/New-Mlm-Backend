...
exports.adminLogin = async (req, res, next) => {
  try {
    ...
    if (!admin) {
      ...
      return res
        .status(401)
        .json({ success: false, message: "Invalid credentials" }); // Use 401 for unauthorized
    }
    ...
    if (!name || !description || !category || Stock === undefined) {
      return res
        ...
        .json({
          success: false,
          message:
            "Missing required product fields (name, description, category, Stock).",
        });
    }
    ...
    if (sponsorId) {
      try {
        ...
        if (sponsor) {
          ...
          await sponsor.save();
          console.log(
            `User ${user.name} added to sponsor ${sponsor.name}'s referrals by admin.`
          );
        } else {
        ...
        console.error(
          `Error updating sponsor ${sponsorId} during admin user creation:`,
          sponsorError
        );
      }
    }
...
exports.editUser = async (req, res) => {
  ...
  try {
    ...
    console.log(
      `Admin ${adminId} attempting to update user ID: ${userIdToUpdate}`
    );

    if (!userIdToUpdate) {
      return res
        .status(400)
        .json({ success: false, message: "User ID parameter is required." });
    }
    ...
    if (req.body.role) {
      console.warn(
        `Admin ${adminId} attempted to change role for user ${userIdToUpdate} via editUser endpoint. Ignoring role change.`
      );
    ...
    if (error.code === 11000) {
      return res
        ...
        .json({
          success: false,
          message: "Update failed: Duplicate key error.",
...
exports.deleteUser = async (req, res) => {
  try {
    ...
    console.log(
      `Admin ${adminId} attempting to delete user ID: ${userIdToDelete}`
    );
    ...
    console.log(
      `User ${userIdToDelete} deleted successfully by admin ${adminId}.`
    );
    ...
    if (user.sponsorId) {
      try {
        await User.updateOne(
          { _id: user.sponsorId },
          { $pull: { referrals: userIdToDelete } }
        );
        console.log(
          `Removed user ${userIdToDelete} from sponsor ${user.sponsorId}'s referrals list.`
        );
      } catch (sponsorUpdateError) {
        console.error(
          `Error removing user ${userIdToDelete} from sponsor ${user.sponsorId}'s list:`,
          sponsorUpdateError
        );
      }
    }
    ...
    res
      ...
      .json({
        success: false,
        message: "Server error deleting user by admin",
        error: error.message,
      });
  }
};
